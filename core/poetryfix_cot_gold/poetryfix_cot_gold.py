...
from core.poetryfix_cot_gold.prompt_templates import *
import re
...
def generate_dataset(poem_csv_filepath: str, poetryfix_cot_gold_csv_filepath: str, save_step=25):
  # Read lucbat dataset
  df = pd.read_csv(poem_csv_filepath).dropna(subset=["content"]).drop_duplicates(subset="content")
  poems = df["content"].to_list()[0: 4000]

  # Read poetryfix_cot_gold.csv file
  if os.path.exists(poetryfix_cot_gold_csv_filepath):
    pfcg_df = pd.read_csv(poetryfix_cot_gold_csv_filepath)
  else:
  ...
  for poem in poems:
    ...
    if (counter >= save_step):
      pfcg_df.to_csv(poetryfix_cot_gold_csv_filepath, index=False, encoding='utf-8-sig')
      print(f"DATASET: {pfcg_df.shape}")
      counter = 0
